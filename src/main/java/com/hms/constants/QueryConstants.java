...
public class QueryConstants {
	...
	public static final String USER_DELETE = "UPDATE t_user SET is_deleted=1 WHERE pk_user_id=#{id} AND is_deleted=0";
	...
	public static final String PATIENT_DELETE = "UPDATE t_patient SET is_deleted=1 WHERE fk_user_id=#{id} AND is_deleted=0";
	...
	public static final String READ_DOCTOR_BY_ID = "<script>SELECT * FROM t_user JOIN t_doctor ON t_user.pk_user_id=t_doctor.fk_user_id WHERE <if test='pk_user_id != 0'> pk_user_id=#{id} AND </if> t_user.is_deleted=0 AND fk_role_id=2 </script>";
	...
	public static final String DOCTOR_DELETE = "UPDATE t_doctor SET is_deleted=1 WHERE fk_user_id=#{id} AND is_deleted=0";

	public static final String GET_PATIENTS_FOR_DOCTOR = "select * from t_user join t_patient on t_user.pk_user_id=t_patient.fk_user_id join t_patient_doctor_mapping on t_user.pk_user_id=t_patient_doctor_mapping.fk_patient_id join t_user as u on t_patient_doctor_mapping.fk_doctor_id=u.pk_user_id where u.is_deleted=0 AND fk_doctor_id=#{id}";
